...
export default function Index(){
    ...
    useEffect(()=>{
        Axios.get("http://localhost:9000/room/getbyhotel/"+id).then((resopne)=>{
            setRoomList(resopne.data);
        });
    },[]);

    const deleteRoom=(idDel)=>{
        Axios.delete("http://localhost:9000/room/delete/"+idDel).then(window.location.href=`/hotel/${id}/room`)
    }
                            ...
                            roomList.map(room=>
                                <tr>
                                    ...
                                    <td style={{width:'115px'}}>
                                        <Button color="success" onClick={()=>updateRoom(room.MaPhong)} style={{width:'100%',marginBottom:'5px'}}>Cập nhật</Button>{' '}
                                        <Button color="danger" onClick={()=>deleteRoom(room.MaPhong)} style={{width:'100%'}}>Xóa</Button>{' '}
                                    </td>
                                </tr>    
                            )
                        }
